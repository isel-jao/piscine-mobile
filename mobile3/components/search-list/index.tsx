...
export function SearchList() {
  ...
  const { data, isLoading } = useSWR(
    ...
    async () => {
      ...
      if (!data.results) {
        setGlobalError("The search returned no results");
      }
      return data.results;
    },
    {
      onError: () => {
        setGlobalError("An error occurred while fetching data");
      },
    }
  );

  const Comp = useCallback(() => {
    if (globalError) return <ThemedText color="danger">{globalError}</ThemedText>;
  ...
  }, [data, isLoading, globalError, search]);
